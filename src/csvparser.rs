...
impl CSVParser {
    ...
    pub fn parse_records(&mut self)
    {
        ...
        reader.deserialize::<Record>().for_each(|record| {
            ...
            log::debug!("{}: parsed a new record == {}", log_header, &unpacked_record);
            self.sender.send(unpacked_record).unwrap();
        });


        log::debug!("{}: all records parsed, sending Finished record", log_header);
        let finish_record = Record{record_type: RecordType::Finished, ..Record::default()};
